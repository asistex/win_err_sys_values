...
PROCEDURE Main()
   ...
   LOCAL oFile := TFileRead():new( "hlp_system_err_codes_0_15841.txt" )
   LOCAL n, d, s, h, x, i := 0

   IF hb_FileExists( "win_sys_err.dbf" )
      Ferase( "win_sys_err.dbf" )
   ENDIF
   Ferase( "win_sys_err.cdx" )

   DBcreate( "win_sys_err.dbf", { {"NUM_ERR","N",10,0},;
                                  {"DEF_ERR","C",65,0},;
                                  {"HEX_ERR","C",10,0},;
                                  {"TXT_ERR","C",650,0};
                                } )

   dbUseArea( .T., "DBFCDX", "win_sys_err", "win_err", .F., .F. )
